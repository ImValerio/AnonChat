...
const Chat = ({ room }) => {
    ...
    return (
        <div className="w-full h-full flex flex-col justify-center items-center  text-center">
            <h1 className="text-5xl font-bold mb-5">Room: <span className=" text-yellow-400 rounded-lg p-1 roomName">{room}</span> </h1>
            <div className="flex flex-grow flex-col justify-start items-center overflow-y-auto shadow-lg max-h-96 m-5 w-min" id="chatMessages">
                {messages.map((msg, i) => {
            ...
            <form onSubmit={(e) => handleSubmit(e)} className="flex flex-grow-0 justify-center items-center w-full ">
                <input className="rounded-l-lg p-4 border-t mr-0 border-b border-l text-gray-800 border-gray-200 bg-white shadow-lg" value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Text message..." required />
